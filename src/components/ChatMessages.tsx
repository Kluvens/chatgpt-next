...
const ChatMessages: React.FC<ChatMessagesProps> = ({ messages, updateMessage, toggleMarkBad }) => {
      ...
      <div className="flex flex-col text-sm md:pb-9">
        ...
        <div className="sticky top-0 p-3 mb-1.5 flex items-center justify-between z-10 h-14 p-2 font-semibold token-surface-primary">
          <div className="flex items-center gap-2 overflow-hidden gap-0">
            <div className="flex items-center">
              ...
              <div role='button' tabIndex={0} className="flex items-center justify-center h-10 rounded-lg px-2 text-gray-500 focus-visible:outline-0 hover:bg-gray-200 focus-visible:bg-gray-200">
                <NewChatIcon />
              </div>
            </div>
            <div role="button" id="radix-:rb1:" aria-haspopup="menu" aria-expanded="false" data-state="closed" className="group flex cursor-pointer items-center gap-1 rounded-xl py-2 px-3 text-lg font-semibold hover:bg-gray-200 radix-state-open:bg-token-main-surface-secondary text-gray-500 rounded-lg py-1.5 overflow-hidden whitespace-nowrap">
              <div className="flex gap-2 text-gray-500">ChatGPT
                <span className="text-gray-500">4o</span>
              </div>
        ...
        {messages.map((message) => {
          if (message.sender === 'user') {
            return <UserMessage key={message.id} message={message} onUpdate={(newText) => updateMessage(message.id, newText)} />;
          } else {
            return <ModelMessage key={message.id} message={message} onToggleMarkBad={() => toggleMarkBad(message.id)} />;
          }
        })}
      </div>
